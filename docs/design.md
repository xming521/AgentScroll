...
标题筛选使用一次模型请求；每个话题在原有知识卡请求中同时完成内容状态、大众可理解性、两项评分以及文案和评论选择。补搜后的话题使用补搜知识卡请求；暂定即时新事件命中历史时沿用现有的单话题重评。热度保底、分数合并和发送策略本身不调用模型。
...
第一轮先识别当前候选中的同事件标题组，再决定入选。同事件标题数达到配置门槛时，优先进入正文采集，不因日常、宣传、含义不明、依赖圈内背景或暂时看不出公共价值而在标题阶段淘汰；不属于 fun 的话题暂按 news 核验。只统计当前候选中的代表标题与关联标题，不计历史召回，也不合并不同事件凑数。门槛由程序从配置填入提示词模板，不作为单独的 JSON 输入字段。
...
默认以取得 3 个可读入口为目标。已有非知乎入口不足时，复用非知乎标题到微博补采；只有整组标题都来自知乎时，才用一次批量模型请求把各组标题转换为 2 至 3 个搜索词，再到微博找同一事件的帖子。搜索词保留能区分事件的主体与动作，去掉泛化问句，不补写标题之外的事实。
...
| 更新评分 | update 的两项评分都只评价旧卡之外的新增进展 | 普通补充默认最高 2.9；值得再次告知的实质变化才可达 3 分以上，重大转折可得大众 4 分；程序不把所有 update 硬性封顶 |
...
| **按需：保存前历史复核** | | |
| 保存前历史复核 | 启用自动分享且即时阈值非空，暂定 new 的大众分或热度保底分达到即时阈值时，本地复核正文历史 | 命中后改为 update、取消热度保底，只重评该话题一次；复用材料、不重新搜索；仅兴趣高分不触发；仍与分享配置耦合 |
| **程序：发送调度与交付** | | |
...
flowchart TD
    ...
    J --> K["按现有触发条件复核历史<br/>命中则改为 update，复用材料重评一次并重算分数"]
...
真实评论可以来自同一话题的任一已采集帖子，不要求与分享链接属于同一帖子。模型只返回来源和评论的本地编号，程序精确回填 URL 和原文；生成评论与平台评论在产物中区分。review 文本中的“网友评论”“我的评论”标签仅供检查，实际发送不加前缀。

### 保存前的历史复核

知识卡和分享分在最终落库前仍是暂定结果。启用自动分享且即时阈值非空时，暂定 new 卡的大众分或热度保底分达到即时阈值，才触发现有的正文历史复核；仅兴趣分高不会触发。

复核使用已采集正文和来源标题，在最近 7 天同类别完整历史事件的标题、当前知识和最新进展中做本地高置信匹配。匹配要求足够的历史信息覆盖、多个共同关键词，且最佳候选明显高于次优候选。

未命中时保留结果；命中时改为 update、附上旧卡和时间线，复用已有材料只重评该话题一次，不重新搜索。此时热度保底失效，以重评结果替换暂定结果。重评失败保留为待补搜状态，不回退发送首次结果。

最终知识状态和分享批次在补搜及必要复核完成后统一保存一次，避免把中间结果送出。该复核仍与分享开关、即时阈值耦合，尚未覆盖全部话题。
