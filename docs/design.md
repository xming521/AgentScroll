...
标题筛选使用一次模型请求；每个话题在原有知识卡请求中同时完成内容状态、大众可理解性、两项评分以及文案和评论选择。补搜后的话题使用补搜知识卡请求；待分享新事件命中历史时进行单话题重评，候选接近时先判断事件关系。热度保底、分数合并和发送策略本身不调用模型。
...
关闭内置通用推荐后，只有明确命中兴趣或达到原有热度入选门槛的话题进入后续流程；程序也按这两个条件过滤首轮结果。news/fun 仍表示内容类型，用于原有内容核验和表达规则；大众分享分固定为 0，兴趣评分、热度保底及发送调度保持原规则。
...
| **按需：保存前历史复核** | | |
| 保存前历史复核 | 启用自动分享，所有已有分享内容的 complete、new 卡均本地复核正文历史 | 候选接近时由模型判断事件关系；命中后改为 update、取消热度保底，重评新增价值；复用材料、不重新搜索 |
| **程序：发送调度与交付** | | |
...
复核使用已采集正文和来源标题，在最近 7 天同类别完整历史事件的标题、当前知识和最新进展中做本地高置信匹配。最佳候选须达到历史信息覆盖和共同关键词门槛。最佳候选明显高于次优候选时直接匹配；候选接近时，将分差范围内最多三条候选的标题、知识和最新进展，以及当前知识摘要和分享文案交给模型判断是否同一事件。允许确认多条历史属于同一事件，不能因历史重复记录导致候选分差小就按新事件放行。

未召回候选或模型确认不是同一事件时保留结果；命中时改为 update、附上旧卡和时间线，并参考其他已确认同事件历史中覆盖的内容，复用已有材料只重评该话题一次，不重新搜索。此时热度保底失效，以重评结果替换暂定结果。事件关系判断失败或重评失败保留为待补搜状态，不回退发送首次结果。

最终知识状态和分享批次在补搜及必要复核完成后统一保存一次，避免把中间结果送出。该复核只覆盖启用自动分享时的待分享新事件。
