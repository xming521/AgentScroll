...
- 不要假设接收分享人了解相关游戏、动漫或影视作品，也不要假设其知道其中的角色、设定和圈内梗。
- 如果不了解这些背景就看不懂笑点，`share_score` 最高为 2.9。
...
`share_score>=3` 时按类别规则填写 `share`。只要有不明显跑题且有真人口语感的真实评论，就必须选择其中最自然的 `comment_id`；全部不合适时再以模仿真人的口吻按规则写一句简短的自然反应`generated_comment`。
