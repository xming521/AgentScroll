...
达到 3 分时，按当前类别专用规则填写分享内容。分享内容应让人知道大致内容，同时想点开原文看完整细节；不得使用材料不支持的夸张表达、伪造悬念或故意隐去改变内容性质的关键事实。分享还必须带一条评论，优先按当前类别专用规则选择现有真实评论；没有合适评论时，再以模仿真人的口吻按规则写一句简短的自然反应。
...
- 按现有材料给每个话题的即时分享价值评分：0 分表示材料不足或没有可用来源；正常评分为 1 至 4 分，最多保留一位小数。1、2、3、4 分分别对应当前类别专用规则中的四档，小数只用于表示相邻档位之间的程度。达到 3 分时，按当前类别专用规则填写即时分享内容。分享内容应让人知道大致内容，同时想点开原文看完整细节；不得使用材料不支持的夸张表达、伪造悬念或故意隐去改变内容性质的关键事实。分享依据可以是第一轮材料或主动搜索材料，但必须选择带 `source_id` 且确实支持分享内容的一项。
