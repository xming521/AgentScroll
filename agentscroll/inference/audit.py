...
DEFAULT_AUDIT_LOG_DIR = Path(
    os.environ.get(AUDIT_LOG_DIR_ENV, project_root() / "logs" / "llm_audit")
).expanduser()
...
def _jsonable(value: Any) -> Any:
    ...
    if isinstance(value, Mapping):
        return {
            str(key): _REDACTED if _is_sensitive_key(key) else _jsonable(item)
    ...
    def _write(self, payload: Mapping[str, Any]) -> None:
        ...
        try:
            ...
            if self.strict:
                raise
            logger.warning(
                f"Failed to write LLM audit event: {type(exc).__name__}: {exc}"
            )
